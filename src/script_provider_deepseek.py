...
class DeepSeekScriptProvider(ScriptProvider):
    ...
    def __init__(
        ...
        *,
        ...
        self.model = str(model or "deepseek-chat").strip() or "deepseek-chat"
        self.base_url = str(base_url or "https://api.deepseek.com").strip() or "https://api.deepseek.com"
    ...
    def _ensure_config(self) -> None:
        if not self.api_key:
            raise DeepSeekProviderError("DEEPSEEK_API_KEY is required for deepseek provider")
    ...
    @staticmethod
    def _read_http_error_body(err: urlerror.HTTPError) -> str:
        try:
    ...
    @staticmethod
    def _is_response_format_unsupported_error(err_body: str) -> bool:
        body = str(err_body or "").lower()
        if not body:
            return False
        mentions_response_format = "response_format" in body or "json_object" in body
        indicates_unsupported = any(
            token in body
            for token in ("unsupported", "not support", "unknown", "unrecognized", "invalid parameter")
        )
    ...
    def _post_json(self, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            err_body = self._read_http_error_body(e)
    ...
    def _extract_content_text(self, response_obj: dict[str, Any]) -> str:
        ...
        if not choices or not isinstance(choices[0], dict):
            raise DeepSeekProviderError("DeepSeek response missing choices[0]")
        msg = choices[0].get("message", {}) if isinstance(choices[0].get("message"), dict) else {}
        content = msg.get("content", "")
        text = str(content or "").strip()
        if not text:
            raise DeepSeekProviderError("DeepSeek response message content is empty")
        return text
    ...
    def parse_provider_response(self, payload: dict[str, Any]) -> ScriptProviderResponse:
        ...
        if not script.title:
            raise DeepSeekProviderError("Parsed response missing script_draft.title")
