...
def get_scene(shot: Dict[str, Any]) -> str:
    parts = _source_parts(shot)
    return parts[0] if len(parts) >= 1 else ""


def get_content(shot: Dict[str, Any]) -> str:
    parts = _source_parts(shot)
    return parts[1] if len(parts) >= 2 else ""


def get_coverage(shot: Dict[str, Any]) -> str:
    parts = _source_parts(shot)
    return parts[2] if len(parts) >= 3 else ""


def get_move(shot: Dict[str, Any]) -> str:
    parts = _source_parts(shot)
...
def is_hero_like(shot: Dict[str, Any]) -> bool:
    tags = set(get_tags(shot))
    if tags & HERO_TAGS:
        return True

    coverage = get_coverage(shot)
    content = get_content(shot)
    primary = get_primary_tag(shot)

    if coverage == "hero":
        return True
    if content in {"building", "brand"} and coverage in {"wide", "hero"}:
        return True
...
def same_content(a: Dict[str, Any], b: Dict[str, Any]) -> bool:
    ca = get_content(a)
    cb = get_content(b)
...
def same_family(a: Dict[str, Any], b: Dict[str, Any]) -> bool:
    return same_scene(a, b) and same_content(a, b)


def coverage_rank(shot: Dict[str, Any]) -> int:
    return COVERAGE_ORDER.get(get_coverage(shot), 99)


def is_progressive_coverage(prev_shot: Dict[str, Any], cur_shot: Dict[str, Any]) -> bool:
    a = get_coverage(prev_shot)
    b = get_coverage(cur_shot)
    if not a or not b:
        return False
...
def _source_filename(shot: Dict[str, Any]) -> str:
    ...
    selected = str(shot.get("selected_file", "") or "").strip()
    if selected:
